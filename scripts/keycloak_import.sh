...
if [ -z "${KEYCLOAK_SERVER_CONF+x}" ]; then
    echo "ERROR: KEYCLOAK_SERVER_CONF variable not provided!"
    exit 1
fi

echo "Importing realm ${KEYCLOAK_SERVER_REALM} from file ${KEYCLOAK_SERVER_CONF}"
"${KCADM}" create partialImport \
    -r "${KEYCLOAK_SERVER_REALM}" -s ifResourceExists=SKIP \
    -o -f "/opt/conf/realms/${KEYCLOAK_SERVER_CONF}".json
